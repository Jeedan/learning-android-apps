...
public class MyShowsListFragment extends ListFragment {
	...
	.inflate(R.menu.shows_item_context, menu);
    }
	...
	case R.id.action_settings:
	    Toast.makeText(getActivity(),
		    "One day this will open the settings window!",
	...
	// downloadShowUpdates(position);
	Log.d(TAG,
		"showName: " + show.getShowName() + " TVDB_ID "
			+ show.getTvDb_id());
	Log.d(TAG,
		"showName: " + show.getShowName() + " tv_Rage_id "
			+ show.getTVRage_id());
	Log.d(TAG,
		"showName: " + show.getShowName() + " next date "
			+ show.getNextReleaseDate());
	Log.d(TAG,
		"showName: " + show.getShowName() + " BANNER URL "
			+ show.getBannerURL());
	Log.d(TAG,
		"showName: " + show.getShowName() + " POSTER URL "
			+ show.getPosterURL());
	...
	protected TVShow doInBackground(Void... params) {
	    ...
	    if ((show.getEpisodeNumber() != null)
		    || (show.getReleaseDate() != null)
		    || (show.getEpisodeTitle() != null)
		    || (show.getSeason() != null)) {
	...
	protected void onPostExecute(TVShow show) {
		...
		mShowDB.saveTrackedShows(mTrackedShows);
		Log.d(TAG, show.getShowName() + " " + show.getEpisodeTitle()
			+ "  got info");
	    } else {
		Toast.makeText(getActivity(), R.string.error_fetched_toast,
			Toast.LENGTH_SHORT).show();
	    }
	    mAdapter.notifyDataSetChanged();
	}
	...
	protected Void doInBackground(Void... params) {
		...
		Bitmap poster = new XMLSerializer().readBitmapFromFile(show
			.getPosterURL());
		if (poster != null) {
		    show.setBanner(poster);
		    Log.d(TAG,
			    "readBitmap doInBackground: " + show.getShowName()
			    + " " + show.getPosterURL());
		} else {
		    new XMLSerializer().downloadPosterandSave(
			    show.getPosterURL(), show, bm);
		    Log.d(TAG,
			    "DownloadBitmap doInBackground: "
				    + show.getShowName() + " "
				    + show.getPosterURL());
		}
	    }
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		convertView = getActivity().getLayoutInflater().inflate(
			R.layout.list_item_tracked_show, null);
	    ...
	    // NETWORK
	    TextView netWorkTextView = (TextView) convertView
		    .findViewById(R.id.network_info_text_view);
	    netWorkTextView.setText(show.getNetwork());

	    // Air day and time
	    TextView airTimeTextView = (TextView) convertView
		    .findViewById(R.id.air_day_time_info_text_view);
	    airTimeTextView.setText(show.getAirTime());

	    // Air Status
	    TextView statusTextView = (TextView) convertView
		    .findViewById(R.id.show_status_text_view);
	    ...
	    seasonEpisodeTextView.setText("Latest ep: " + show.getSeason()
	    ...
	    showImage = (ImageView) convertView
		    .findViewById(R.id.show_icon_imageView);
 ...
 * 
 * 
 * old //download metthod private class FetchShowInfo extends AsyncTask<Void,
 * Void, ArrayList<TVShow>> {
 * 
 * private ProgressDialog dialog;
 * 
 * public FetchShowInfo(Context context) { dialog = new ProgressDialog(context);
 * }
 * 
 * @Override protected ArrayList<TVShow> doInBackground(Void... params) { //
 * fetch either the latest episode or all episodes for a show ArrayList<TVShow>
 * fetchedShow = new ArrayList<TVShow>(); for (int i = 0; i <
 * mTrackedShows.size(); i++) { String showName =
 * mTrackedShows.get(i).getShowName().replaceAll("\\s", "");
 * 
 * if (!(mTrackedShows.get(i).getUpdatedInformation())) { if
 * (showName.contains("The")) { String cutThe = showName.substring(3); TVShow ep
 * = new XMLSerializer().fetchItem(cutThe); ep.setUpdatedInformation(true);
 * fetchedShow.add(ep); } else { TVShow ep = new
 * XMLSerializer().fetchItem(showName); ep.setUpdatedInformation(true);
 * fetchedShow.add(ep); } }else { fetchedShow.add(mTrackedShows.get(i)); } }
 * mReturnedShows = fetchedShow.size() > 0; return fetchedShow; }
 * 
 * @Override protected void onPreExecute() {
 * dialog.setMessage("Loading show information"); dialog.show(); }
 * 
 * @Override protected void onPostExecute(ArrayList<TVShow> shows){
 * super.onPostExecute(shows); if(dialog.isShowing()) dialog.dismiss(); // close
 * the dialog
 * 
 * if(mReturnedShows) { mTrackedShows = shows; Toast.makeText(getActivity(),
 * R.string.fetched_info_toast, Toast.LENGTH_SHORT).show();
 * mShowDB.saveTrackedShows(mTrackedShows);
 * mShowDB.setTrackedShows(mTrackedShows); ShowDatabase.mHasLoadedTracked =
 * true; }else { ShowDatabase.mHasLoadedTracked = false;
 * Toast.makeText(getActivity(), R.string.error_fetched_toast,
 * Toast.LENGTH_SHORT).show(); } setupAdapter(); } }
 */
